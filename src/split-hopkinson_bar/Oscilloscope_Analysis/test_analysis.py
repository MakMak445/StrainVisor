...
# ==========================================
# 1. FIXED PEAK DETECTION (WITH FALLBACK)
# ==========================================
def first_contact_auto(
    ...
    if ref_or_trans == 'transmission':
        ...
        strain_peaks, properties = find_peaks(y_s, prominence=(0.1, None), plateau_size=True, height=(5*high_thr, None), distance=5000)
...
def process_single_file(filepath, output_directory):
    ...
    # Transmission (HEAVY SMOOTHING ADDED HERE: sg_win=151)
    trans_idx_L, trans_idx_R, trans_t_L, trans_t_R, trans_mu = first_contact_auto(
        data["Time (ms)"], data[col_d], 3, 'prominences', 'transmission', sg_win=151
    )

    # Max Transmission Time Gate
    if len(ref_t_L) > 0 and len(trans_t_L) > 0:
        t_incident = ref_t_L[0]
        filtered_trans_idx_L, filtered_trans_idx_R, filtered_trans_t_L, filtered_trans_t_R = [], [], [], []
...
if __name__ == "__main__":
    ...
    FILE_TO_FIX = "/home/MakMak445/projects/StrainVisor/src/split-hopkinson_bar/Oscilloscope_Analysis/picoscope_csv/1d1bar.csv"
    
    # Save it directly into your baseline folder so delay_analysis.py finds it immediately
    OUTPUT_FOLDER = "/home/MakMak445/projects/StrainVisor/src/split-hopkinson_bar/Oscilloscope_Analysis/overlay_results3/1d1_base"
