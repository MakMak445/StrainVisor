...
GAUGE_FACTOR = 2.04   # Strain Gauge Factor
...
def process_and_overlay(filepath, output_directory):
    ...
    if "Channel C (V)" in data.columns:
        ...
        print(f"Skipping {os.path.basename(filepath)}: Could not find Channel C in V or mV.")
        return
    ...
    if "Channel D (V)" in data.columns:
        ...
        print(f"Skipping {os.path.basename(filepath)}: Could not find Channel D in V or mV.")
        return

    # --- 1. Get Incident and Reflection Pulses (Channel C) ---
    ref_idx_L, ref_idx_R, ref_t_L, ref_t_R, reflect_mu = first_contact_auto(
        data["Time (ms)"], data[col_c], 5, 'prominences', 'reflection'
    )
    
    # --- 2. Get Transmission Pulses (Channel D) ---
    trans_idx_L, trans_idx_R, trans_t_L, trans_t_R, trans_mu = first_contact_auto(
        data["Time (ms)"], data[col_d], 3, 'prominences', 'transmission'
    )

    # ==========================================
    # --- MAX TRANSMISSION TIME GATE ---
    # ==========================================
    MAX_TRANS_DELAY_MS = 0.25  # Adjust this if needed

    if len(ref_t_L) > 0 and len(trans_t_L) > 0:
        t_incident = ref_t_L[0]
        
        filtered_trans_idx_L = []
        filtered_trans_idx_R = []
        filtered_trans_t_L = []
        filtered_trans_t_R = []
        
        for i in range(len(trans_t_L)):
            if trans_t_L[i] <= (t_incident + MAX_TRANS_DELAY_MS):
        ...
        trans_idx_L = filtered_trans_idx_L
        trans_idx_R = filtered_trans_idx_R
        trans_t_L = filtered_trans_t_L
        trans_t_R = filtered_trans_t_R

    # ==========================================
    # --- NEW FAIL-SAFE LOGIC ---
    # ==========================================
    has_incident = len(ref_idx_L) > 0
    
    if not has_incident:
        print(f"Skipping {os.path.basename(filepath)}: Could not detect the initial Incident pulse.")
        return
        
    has_reflection = len(ref_idx_L) > 1
    has_transmission = len(trans_idx_L) > 0

    if not (has_reflection or has_transmission):
        print(f"Skipping {os.path.basename(filepath)}: Found Incident, but missing BOTH Reflection and Transmission.")
        return

    # ==========================================
    # --- TRUNCATE TO FIRST PULSES ONLY ---
    # ==========================================
    # Cap Channel C at 2 pulses: Incident [0] and 1st Reflection [1]
    ref_idx_L = ref_idx_L[:2]
    ref_idx_R = ref_idx_R[:2]
    
    # Cap Channel D at 1 pulse: 1st Transmission [0]
    trans_idx_L = trans_idx_L[:1]
    trans_idx_R = trans_idx_R[:1]
    
    # Re-evaluate flags after truncation
    has_reflection = len(ref_idx_L) > 1
    has_transmission = len(trans_idx_L) > 0

    # --- Setup Plot and Dictionary for CSV ---
    fig, ax = plt.subplots(figsize=(12, 7))
    pulse_data_for_csv = {}
    max_pulse_length = 0
    shared_time_axis = None

    # --- Process Incident and Reflection Pulses (Channel C) ---
    ...
    # Inject empty columns if reflection is missing
    if not has_reflection:
        print("  -> No reflection pulse detected. Injecting empty columns.")
        pulse_data_for_csv['Reflected Pulse 1 Time (ms)'] = []
        pulse_data_for_csv['Reflected Pulse 1 Strain'] = []

    # --- Process Transmission Pulses (Channel D) ---
    if not has_transmission:
        print("  -> No transmission pulse detected. Injecting empty columns.")
        pulse_data_for_csv['Transmission 1 Time (ms)'] = []
        pulse_data_for_csv['Transmission 1 Strain'] = []
    else:
    ...
    if cont == 'q' or cont == 'quit':
        ...
        # Save CSV (Padding missing data with NaNs)
...
if __name__ == "__main__":
    ...
    output_folder = "/home/MakMak445/projects/StrainVisor/src/split-hopkinson_bar/Oscilloscope_Analysis/overlay_results3"
